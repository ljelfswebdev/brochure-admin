...
function RowEditor({ row, onChange, onRemove }) {
  function setColumns(cols){
    ...
    if (columns===1) fields = fields.slice(0,1).length?fields.slice(0,1):[{ label:'', type:'text' }];
    if (columns===2) fields = (fields.concat([{},{ }])).slice(0,2).map(f=>({ label:'', type:'text', ...f }));
  ...
  const fields = (columns===1)
    ? (row.fields||[]).slice(0,1).length ? (row.fields||[]).slice(0,1) : [{ label:'', type:'text' }]
    : (row.fields||[]).concat([{},{ }]).slice(0,2).map(f=>({ label:'', type:'text', ...f }));
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      try{
        ...
        setAutoReply(d?.autoReply || { enabled:false, subject:'', message:'', fromName:'', fromEmail:'' });
      }catch(e){
